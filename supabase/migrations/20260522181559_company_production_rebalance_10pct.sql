-- AstraCorp - rebalance de produccion de empresas
-- Reduce la produccion base de todas las empresas productivas un 10%.
...
begin
  return case p_type
    ...
    when 'solar_panel' then jsonb_build_object('key','solar_panel','name','Panel solar','buildCost',54,'resourceKey','energy_cells','resourceLabel','Energia','ratePerHour',8.55,'maxStorage',56,'unlockLevel',2,'inputs','[]'::jsonb)
    ...
    when 'water_purifier' then jsonb_build_object('key','water_purifier','name','Planta de purificacion','buildCost',152,'resourceKey','purified_water','resourceLabel','Agua purificada','ratePerHour',5.22,'maxStorage',40,'unlockLevel',4,'inputs',jsonb_build_array(jsonb_build_object('key','water','amount',2),jsonb_build_object('key','energy_cells','amount',1)))
    when 'smelter' then jsonb_build_object('key','smelter','name','Fundicion','buildCost',210,'resourceKey','metal_components','resourceLabel','Componentes metalicos','ratePerHour',4.32,'maxStorage',36,'unlockLevel',5,'inputs',jsonb_build_array(jsonb_build_object('key','mineral','amount',2),jsonb_build_object('key','energy_cells','amount',1)))
    when 'electrolysis_plant' then jsonb_build_object('key','electrolysis_plant','name','Planta de electrolisis','buildCost',420,'resourceKey','oxygen_tanks','resourceLabel','Tanques de oxigeno','ratePerHour',2.52,'maxStorage',24,'unlockLevel',6,'inputs',jsonb_build_array(jsonb_build_object('key','purified_water','amount',2),jsonb_build_object('key','energy_cells','amount',2)))
    when 'industrial_forge' then jsonb_build_object('key','industrial_forge','name','Forja industrial','buildCost',520,'resourceKey','alloy_frames','resourceLabel','Estructuras de aleacion','ratePerHour',2.16,'maxStorage',22,'unlockLevel',7,'inputs',jsonb_build_array(jsonb_build_object('key','metal_components','amount',2),jsonb_build_object('key','mineral','amount',1),jsonb_build_object('key','energy_cells','amount',2)))
    when 'habitat_factory' then jsonb_build_object('key','habitat_factory','name','Fabrica de habitats','buildCost',760,'resourceKey','habitat_modules','resourceLabel','Modulos de habitat','ratePerHour',1.26,'maxStorage',14,'unlockLevel',9,'inputs',jsonb_build_array(jsonb_build_object('key','oxygen_tanks','amount',1),jsonb_build_object('key','alloy_frames','amount',1),jsonb_build_object('key','purified_water','amount',1)))
