...
export function MarketView({ market, inventory, onBuy, onSell, player, companies = [], activeEvent = null }) {
  ...
  return (
    <div style={styles.wrapper}>
      {false && <div style={{ ...styles.recommendation, padding: 12, borderRadius: 10 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <div>
            <div style={{ fontSize: 11, color: '#7dd3fc', fontWeight: 900, letterSpacing: 0.7 }}>MERCADO</div>
            <div style={{ color: '#f8fafc', fontSize: 18, fontWeight: 900 }}>
              {bestSell ? `Mejor venta: ${bestSell.name}` : bestBuy ? `Mejor compra: ${bestBuy.name}` : 'Sin oportunidad clara'}
            </div>
            <div style={{ fontSize: 12, color: '#94a3b8' }}>
              {formatNumber(credits, 2)} cr · {items.length} recursos · {hotCount} calientes · {formatNumber(totalInventoryUnits)} uds en stock
            </div>
          </div>
          <div style={{ display: 'grid', gap: 3, minWidth: 190, fontSize: 12, color: '#cbd5e1' }}>
            <span>{bestSell ? `Vende ${bestSell.name} a ${formatNumber(bestSell.sellPrice, 2)} cr/u` : 'No hay stock vendible.'}</span>
            <span>{bestBuy ? `Compra vigilada: ${bestBuy.name} (${formatNumber(bestBuy.heat, 2)}x)` : 'Saldo sin compra recomendada.'}</span>
          </div>
        </div>
      </div>}

      {false && marketSpotlight && (
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(160px, 1fr) repeat(3, minmax(90px, 0.65fr))', gap: 8, alignItems: 'center', padding: 10, borderRadius: 10, background: 'rgba(15,23,42,0.76)', border: '1px solid rgba(56,189,248,0.16)' }}>
          <div>
            <div style={{ fontSize: 11, color: '#94a3b8', fontWeight: 900 }}>Oportunidad</div>
            <div style={{ color: '#f8fafc', fontWeight: 900 }}>{marketSpotlight.name}</div>
          </div>
          <div style={{ color: '#cbd5e1', fontSize: 12 }}>Presion <strong>{formatNumber(marketSpotlight.heat, 2)}x</strong></div>
          <div style={{ color: '#cbd5e1', fontSize: 12 }}>Venta <strong>{formatNumber(marketSpotlight.sellPrice, 2)} cr</strong></div>
          <div style={{ color: '#cbd5e1', fontSize: 12 }}>Stock <strong>{formatNumber(marketSpotlight.owned)}</strong></div>
        </div>
      )}
    ...
    <div style={styles.wrapper}>
      {Number(player?.level ?? 1) <= 2 && (
        <div style={styles.helperBox}>
          <strong>Que mirar aqui:</strong> el mercado todavia no es tu prioridad principal. Entra
          sobre todo para entender precios y hacer tu primer intercambio cuando ya tengas una base
          minima de trabajos y al menos una empresa funcionando.
        </div>
      )}
      ...
      <div style={styles.heroGrid}>
        <HeroStat
          label="Créditos"
          value={formatNumber(credits, 2)}
          sub="Liquidez actual"
        />
        <HeroStat
          label="Recursos en mercado"
          value={items.length}
          sub="Opciones disponibles"
        />
        <HeroStat
          label="Mercados calientes"
          value={hotCount}
          sub="Buena señal para vender"
        />
        <HeroStat
          label="Inventario total"
          value={formatNumber(totalInventoryUnits)}
          sub="Unidades almacenadas"
        />
      </div>
      ...
      {marketSpotlight && (
        <div style={styles.spotlight}>
          <div style={styles.spotlightTop}>
            <div>
              <div style={styles.spotlightTitle}>Oportunidad del ciclo</div>
              <div style={styles.spotlightText}>
                {marketSpotlight.name} concentra la mejor mezcla de precio y presion de demanda en
                este momento.
              </div>
            </div>
            <div
              style={{
                ...
                color: marketSpotlight.heat >= 1.25 ? '#bbf7d0' : '#bfdbfe',
              }}
            >
              {marketSpotlight.heat >= 1.25 ? 'Vender ahora' : 'Vigilar mercado'}
            </div>
          </div>

          <div style={styles.spotlightGrid}>
            <div style={styles.spotlightBox}>
              <div style={styles.spotlightLabel}>Recurso</div>
              <div style={styles.spotlightValue}>{marketSpotlight.name}</div>
            </div>
            <div style={styles.spotlightBox}>
              <div style={styles.spotlightLabel}>Presion</div>
              <div style={styles.spotlightValue}>{formatNumber(marketSpotlight.heat, 2)}x</div>
            </div>
            <div style={styles.spotlightBox}>
              <div style={styles.spotlightLabel}>Venta unitaria</div>
              <div style={styles.spotlightValue}>€{formatNumber(marketSpotlight.sellPrice, 2)}</div>
            </div>
            <div style={styles.spotlightBox}>
              <div style={styles.spotlightLabel}>Stock actual</div>
              <div style={styles.spotlightValue}>{formatNumber(marketSpotlight.owned)}</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={styles.recommendation}>
        {bestSell
          ? `Mejor venta ahora: ${bestSell.name} por €${formatNumber(bestSell.sellPrice, 2)} la unidad.`
          : 'Todavía no tienes stock para aprovechar el mercado.'}{' '}
        {bestBuy
          ? `Mejor compra ahora: ${bestBuy.name}, con presion ${formatNumber(bestBuy.heat, 2)}x.`
          : 'Ahora mismo no hay compras claras dentro de tu saldo.'}
      </div>

      <div style={styles.sectionTitle}>💹 Mercado orbital</div>

      {activeEvent && activeEventItems.length > 0 && (
        <div style={styles.eventOpportunity}>
          <strong>{activeEvent.title}:</strong> {activeEventItems.map(([, item]) => item?.name).join(', ')}
          {' '}tienen demanda alterada ahora. Recomendacion: vende si tienes stock; compra solo si necesitas cerrar contratos.
        </div>
      )}
      ...
      <div style={styles.grid}>
        {items.map(([key, item]) => {
          ...
          const sellPrice = Number(item?.price ?? 0) * 0.78;
          const maxBuy = getMaxAffordableBuy(item, credits);
          const buyTotal = buyQuote.total;
          const canBuy = credits >= buyTotal;
          const canSell = owned > 0;
          ...
          const isHot = state.tone === 'hot';
          const isCold = state.tone === 'cold';
          const eventAffected = activeEventKeys.has(key);

          const cardStyle = {
            ...styles.card,
            ...(isHot ? styles.cardHot : null),
            ...(isCold ? styles.cardCold : null),
            ...(eventAffected ? styles.eventCard : null),
          };

          const cardEvents = floatingEvents.filter((ev) => ev.key === key);
          const flow = resourceFlow[key] || null;
          const topConsumers = (flow?.consumers || [])
            ...
            <div key={key} style={cardStyle}>
              ...
              <div style={styles.floatWrap}>
                {cardEvents.map((ev) => (
                  <div
                    key={ev.id}
                    style={ev.type === 'buy' ? styles.floatTextBuy : styles.floatTextSell}
                  >
                    {ev.text}
                  </div>
                ))}
              ...
              <div style={styles.header}>
                <div style={styles.titleBlock}>
                  ...
                  <div style={styles.subtitleRow}>
                    <span style={styles.pill}>{owned} en inventario</span>
                    <span style={styles.pill}>{formatNumber(marketSupply)} en mercado</span>
                    {priceLimitState ? (
                      <span style={styles.eventTag}>
                        {priceLimitState === 'min' ? 'Precio minimo' : 'Precio maximo'}
                      </span>
                    ) : null}
                    <span style={styles.pill}>{state.hint}</span>
                    <span style={styles.pill}>Max {MARKET_MAX_TRADE_UNITS}/operacion</span>
                    {eventAffected ? (
                      <span style={styles.eventTag}>{activeEvent.title}</span>
                    ) : null}
                    {flow?.amountPerHour > 0 ? (
                      <span style={styles.flowTag}>
                        Consumo auto: {formatNumber(flow.amountPerHour, 2)}/h
                      </span>
                    ) : null}
                  </div>
                </div>
                ...
                <div style={styles.priceCol}>
                  <div style={styles.priceBadge}>€{formatNumber(buyDisplayPrice, 2)}</div>

                  <div
                    style={{
                      ...styles.stateBadge,
                      ...getStateBadgeStyle(state.tone),
                    }}
                  >
                    {state.label}
                  </div>
              ...
              <div style={styles.heroLine}>
                <div style={styles.heroStat}>
                  <div style={styles.heroStatLabel}>Precio compra</div>
                  <div style={styles.heroStatValue}>€{formatNumber(buyDisplayPrice, 2)}</div>
                </div>

                <div style={styles.heroStat}>
                  <div style={styles.heroStatLabel}>Precio venta</div>
                  <div style={styles.heroStatValue}>€{formatNumber(sellPrice, 2)}</div>
                </div>
              </div>
              ...
              <div style={styles.trendWrap}>
                <div style={styles.trendTop}>
                  ...
                  <span>{formatNumber(heat, 2)}x</span>
                </div>
              ...
              <div style={styles.statGrid}>
                <div style={styles.statBox}>
                  ...
                  <div style={styles.statValue}>{formatNumber(owned)}</div>
                </div>

                <div style={styles.statBox}>
                  <div style={styles.statLabel}>Puedes comprar</div>
                  <div style={styles.statValue}>{canBuy ? 'Sí' : 'No'}</div>
                </div>

                <div style={styles.statBox}>
                  <div style={styles.statLabel}>En mercado</div>
                  <div style={styles.statValue}>{formatNumber(marketSupply)}</div>
                </div>

                <div style={styles.statBox}>
                  <div style={styles.statLabel}>Valor venta total</div>
                  <div style={styles.statValue}>€{formatNumber(owned * sellPrice, 2)}</div>
                </div>

                <div style={styles.statBox}>
                  <div style={styles.statLabel}>Demanda</div>
                  <div style={styles.statValue}>{formatNumber(item?.demand ?? 0)}</div>
                </div>

                <div style={styles.statBox}>
                  <div style={styles.statLabel}>Consumo auto</div>
                  <div style={styles.statValue}>
                    {flow?.amountPerHour > 0 ? `${formatNumber(flow.amountPerHour, 2)}/h` : '0/h'}
                  </div>
                </div>
              </div>
              ...
              <div style={styles.helperBox}>
                {helperText}
                {` Impacto de volumen: compra ${formatNumber(buyQuote.amount)} uds por ${formatNumber(buyQuote.total, 2)} creditos; venta ${formatNumber(sellQuote.amount)} uds por ${formatNumber(sellQuote.total, 2)} creditos.`}
                {flow?.amountPerHour > 0
                  ? ` Tu cadena está gastando este recurso automáticamente. Lo consumen: ${topConsumers}.`
                  : ''}
              </div>

              {eventAffected ? (
                <div style={styles.eventOpportunity}>
                  Evento activo: este recurso tiene demanda temporal. Si tienes stock, vender o cerrar contratos suele ser mejor que esperar.
                </div>
              ) : null}
              ...
              <div style={styles.quantityPanel}>
                ...
                <button
                  type="button"
                  onClick={() => setQuantity(key, Math.max(1, maxBuy))}
                  disabled={maxBuy <= 0}
                  style={{
                    ...styles.quantityMiniButton,
                    ...(maxBuy <= 0 ? styles.buttonDisabled : null),
                  }}
                >
                  Max compra
                </button>
                <button
                  type="button"
                  onClick={() => setQuantity(key, Math.max(1, owned))}
                  disabled={owned <= 0}
                  style={{
                    ...styles.quantityMiniButton,
                    ...(owned <= 0 ? styles.buttonDisabled : null),
                  }}
                >
                  Max venta
                </button>
