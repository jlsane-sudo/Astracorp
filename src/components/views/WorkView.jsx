    ...
    <div style={styles.wrapper}>
      {playerLevel <= 2 && (
        <div style={styles.helperBox}>
          <strong>Guia rapida:</strong> usa esta pestaña para arrancar. Tu objetivo temprano aquí es
          completar algunos trabajos, reunir créditos y alcanzar el nivel 2 antes de abrir más
          sistemas.
        </div>
      )}

      <div style={styles.sectionTitle}>Operaciones disponibles</div>
      ...
      <div style={styles.grid}>
        {(jobs || []).map((job) => {
          ...
          const cardStyle = {
            ...styles.card,
            ...(isThisActive ? styles.cardActive : null),
            ...(!canStart && !isThisActive ? styles.cardBlocked : null),
          };

          return (
            <div key={job.label} style={cardStyle}>
              ...
              <div style={styles.header}>
                <div style={styles.titleBlock}>
                  ...
                  <div style={styles.subtitle}>Requiere nivel {formatNumber(job?.unlockLevel ?? 1)}</div>
                </div>

                <div style={styles.badgeColumn}>
                  <div style={styles.rewardBadge}>
                    +{formatNumber(resourceOutput)} {getResourceLabel(job?.item)}
                  </div>

                  {operationRewards.length > 0 && (
                    <div style={styles.operationRewardRow}>
                      {operationRewards.map((reward) => (
                        <span key={reward.key} style={styles.operationRewardBadge}>
                          +{formatNumber(reward.amount, 2)} {getOperationRewardLabel(reward.key)}
                        </span>
              ...
              <div
                style={{
                  ...styles.visualShell,
                  background: profile.shellBg,
                  border: `1px solid ${profile.shellBorder}`,
                  ...(isThisActive ? styles.visualShellActive : null),
                }}
              >
                <IndustryVisual
                  type={getJobVisualType(job?.item)}
                  active={isThisActive}
                  size={54}
                />
                <div style={styles.visualText}>
                  <div style={styles.visualTitle}>{getResourceLabel(job?.item)}</div>
                  <div
                    style={{
                      ...styles.modePill,
                  ...
                  <div style={styles.telemetryRow}>
                    {profile.bars.map((bar, index) => (
                      <div
                        key={`${job.label}-bar-${index}`}
                        style={{
                          ...styles.telemetryBar,
                          height: `${bar}%`,
                          background: `linear-gradient(180deg, ${profile.accent}, rgba(255,255,255,0.22))`,
                        }}
                      />
                    ))}
                  </div>
                  <div style={styles.visualSub}>
                    {isThisActive ? `Proceso activo. Al terminar: ${getOperationRewardText(job)}` : `${profile.hint} Territorio: ${getOperationRewardText(job)}`}
                  </div>
                </div>
              </div>
              ...
              <div style={styles.heroLine}>
                <div style={styles.heroStat}>
                  <div style={styles.heroLabel}>Produccion manual</div>
                ...
                <div style={styles.heroStat}>
                  <div style={styles.heroLabel}>Duracion</div>
                  <div style={styles.heroValue}>{formatDuration(durationSec)}</div>
                </div>
              </div>

              <div
                style={{
                  ...styles.activeInlineBox,
                  ...(isThisActive
                    ? {
                        background:
                          'linear-gradient(180deg, rgba(56,189,248,0.09), rgba(15,23,42,0.58))',
              ...
              >
                <div style={styles.progressWrap}>
                  <div style={styles.progressTop}>
                    <span>{isThisActive ? `Progreso: ${currentProgress.toFixed(0)}%` : 'En espera'}</span>
                    <span>
                      {isThisActive
                        ? `Restante: ${formatRemaining(currentRemainingSec)}`
                        : `Duracion: ${formatDuration(durationSec)}`}
                    </span>
                  </div>
                  ...
                  <div style={styles.progressOuter}>
                    <div
                      style={{
                        ...
                        width: `${isThisActive ? currentProgress : 0}%`,
                        opacity: isThisActive ? 1 : 0.35,
                      }}
                    />
                  </div>
                </div>
                ...
                <div style={styles.progressHint}>
                  {isThisActive
                    ? 'El trabajo ya esta corriendo en esta misma tarjeta.'
                    : 'Aqui aparecera la barra y el tiempo cuando inicies este trabajo.'}
                </div>

                <button
                  type="button"
                  onClick={handleWorkAdBoost}
                  disabled={!isThisActive || Boolean(activeJob?.adBoostUsed)}
                  style={{
                    ...styles.adButton,
                    ...(!isThisActive || activeJob?.adBoostUsed ? styles.buttonDisabled : null),
                  }}
                >
                  {!isThisActive
                    ? 'Boost disponible al iniciar'
                    : activeJob?.adBoostUsed
                    ? 'Impulso usado'
                    : 'Ver anuncio: acelerar trabajo'}
                </button>
              </div>

              <div style={styles.statGrid}>
                <div style={styles.statBox}>
                  <div style={styles.statLabel}>Coste energia</div>
                  <div style={styles.statValue}>{formatNumber(job?.cost ?? 0)}</div>
                </div>

                <div style={styles.statBox}>
                  <div style={styles.statLabel}>Equivale / hora</div>
                  <div style={styles.statValue}>
                    {formatNumber(manualRatePerHour, 1)} {getResourceLabel(job?.item)}
                  </div>
                </div>

                <div style={styles.statBox}>
                  <div style={styles.statLabel}>XP</div>
                  <div style={styles.statValue}>{formatNumber(job?.xp ?? 0)}</div>
                </div>

                <div style={styles.statBox}>
                  <div style={styles.statLabel}>Bonus sector</div>
                  <div style={styles.statValue}>
                    {sectorBonusRate > 0 ? `+${formatNumber(sectorBonusRate * 100, 1)}%` : '0%'}
                  </div>
                </div>

                <div style={styles.statBox}>
                  <div style={styles.statLabel}>Operativo</div>
                  <div style={styles.statValue}>
                    {operationRewards.length > 0
                      ? operationRewards.map((reward) => `${formatNumber(reward.amount, 2)} ${getOperationRewardLabel(reward.key)}`).join(' + ')
                      : '0'}
                  </div>
                </div>
              </div>
              ...
              <div style={styles.helperBox}>
                {isThisActive
                  ? `Produciendo manualmente ${getResourceLabel(job?.item)}.`
                  : state.tone === 'danger'
                  ? 'Sube de nivel para desbloquearlo.'
                  : state.tone === 'warn'
                  ? 'Te falta energia.'
                  : activeJob
                  ? 'Ya hay otra operacion activa.'
                  : `Trabajo manual: genera ${getResourceLabel(job?.item)} a un ritmo aprox. de ${formatNumber(manualRatePerHour, 1)}/h. Bonus territorial aplicado: ${formatNumber(sectorBonusRate * 100, 1)}%.`}
              </div>
              ...
              <button
                ...
                style={{
                  ...styles.button,
                  ...(!canStart ? styles.buttonDisabled : null),
                }}
              >
                {isThisActive
                  ? 'Trabajo en curso'
