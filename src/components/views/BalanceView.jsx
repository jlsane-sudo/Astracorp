...
export function BalanceView({ save, onOpenTab }) {
  ...
  return (
    <div style={styles.wrap}>
      <div>
        <div className="pw-section-kicker">BALANCE</div>
        <div style={{ color: '#f8fafc', fontSize: 18, fontWeight: 900 }}>Panel interno de economia</div>
      </div>

      <AutonomyStat runwayHours={runwayHours} hasMaintenance={balance.totalMaintenance > 0} />
      <div style={styles.grid}>
        <Stat label="Produccion total/h" value={fmt(balance.totalProduction)} sub="Suma de empresas hidratadas" />
        <Stat label="Mantenimiento/h" value={`${fmt(balance.totalMaintenance)} cr`} sub="Coste operativo por hora" />
        <Stat label="Contratos listos" value={balance.readyContracts} sub={`${balance.expiringContracts} cerca de expirar`} />
        <Stat label="Creditos" value={fmt(credits)} sub={`Deuda: ${fmt(save?.maintenanceDebt || 0)} cr`} />
      </div>

      <div style={styles.card}>
        <div style={styles.label}>Diagnostico operativo</div>
        <div style={styles.insightGrid}>
          <InsightItem
            label="Recurso bloqueante"
            value={insight.bottleneck?.name || 'Sin datos'}
            sub={insight.bottleneck ? `Stock ${fmt(insight.bottleneck.stored)} - neto ${fmt(insight.bottleneck.net)}/h` : 'Construye empresas para medir flujo'}
          />
          <InsightItem
            label="Tiempo hasta atasco"
            value={insight.bottleneck ? formatHours(insight.bottleneck.timeToEmpty) : '--'}
            sub={`${insight.negativeCount} recurso(s) en negativo`}
          />
          <InsightItem
            label="Siguiente empresa"
            value={insight.producer?.name || 'Ninguna'}
            sub={insight.producer ? `${insight.producerStatus} - coste ${fmt(insight.producerCost, 0)} cr` : insight.producerStatus}
          />
          <InsightItem
            label="Mejor accion"
            value="Prioridad"
        ...
        <div style={styles.actionRow}>
          <button type="button" style={styles.actionButton} onClick={() => onOpenTab?.('business')}>
            Ajustar empresas
          </button>
          <button type="button" style={styles.actionButton} onClick={() => onOpenTab?.('missions')}>
            Revisar contratos
          </button>
          <button type="button" style={styles.actionButton} onClick={() => onOpenTab?.('market')}>
            Abrir mercado
          </button>
        </div>
      </div>
      ...
      {nextPlanet && (
        <div style={styles.card}>
          <div style={styles.label}>Siguiente planeta</div>
          <div style={styles.value}>{nextPlanet.name}</div>
          <div style={styles.sub}>
            Faltan: {nextPlanetStatus?.missing?.length ? nextPlanetStatus.missing.join(', ') : 'listo para viajar'}.
          </div>
        </div>
      )}

      <div style={styles.sectionTitle}>Simulador rapido</div>
      <div style={styles.grid}>
        {simulations.map((sim) => {
          const critical = sim.resources[0];
          return (
            <Stat
              key={sim.hours}
              label={`+${sim.hours}h`}
              value={`${fmt(sim.maintenance)} cr`}
              sub={`${sim.readyAfter} contrato(s) posibles. Critico: ${critical ? `${critical.name} ${fmt(critical.projected)}` : 'sin flujo'}`}
            />
          );
        })}
      </div>
      ...
      <div style={styles.table}>
        <div style={{ ...styles.row, ...styles.head }}>
          <div>Empresa</div><div>Cantidad</div><div>Prod/h</div><div>Mant/h</div><div>Stock</div>
        </div>
        {balance.rows.map((row) => (
          <div key={row.meta.key} style={styles.row}>
            <div>{row.meta.icon} {row.meta.name}</div>
            <div>{row.count}{row.paused ? ` (${row.paused} pausa)` : ''}</div>
            <div>{fmt(row.production)}</div>
            <div>{fmt(row.maintenance)}</div>
            <div>{fmt(row.stored)}</div>
          </div>
        ))}
      </div>

      <div style={styles.sectionTitle}>Historial economico</div>
      <div style={styles.table}>
        {history.length === 0 ? (
          <div style={styles.card}><div style={styles.sub}>Aun no hay movimientos registrados.</div></div>
        ) : history.map((entry) => (
          <div key={`${entry.t}-${entry.label}`} style={styles.row}>
            <div>{entry.label}</div>
            <div>{safeDateLabel(entry?.t)}</div>
            <div style={{ color: Number(entry.credits || 0) >= 0 ? '#86efac' : '#fca5a5' }}>
              {Number(entry.credits || 0) >= 0 ? '+' : ''}{fmt(entry.credits)} cr
            </div>
            <div>{entry.type}</div>
